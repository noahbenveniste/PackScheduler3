...
public class StudentDirectory {
	...
	/**
	 * Return a Student based off of ID
	 * 
	 * @param id of student to find
	 * @return Student that has the same ID as the parameter
	 */
    public Student getStudentById(String id) {
        ...
        for (int i = 0; i < studentDirectory.size(); i++) {
            ...
            // If the ID for the Student is equal to the parameter, return that student
            if(id.equals(studentDirectory.get(i).getId())) {
                return studentDirectory.get(i);
            } 
        }
        
        // If no student has that has an ID equal to the parameter, return null
        return null;
    }
}
