...
public class CourseRoll {
     ...
     * 
     * @param c
     *            the Course being modified
     * @param enrollmentCap
     *            enrollment cap for the class
     */
     ...
     *             if enrollmentCap is larger/smaller than the allowed enrollment
     */
    ...
    /**
     * Get the OpenSeats in the class based on the difference between enrollmentCap
     * and the size of roll
     * 
     ...
     * 
     * @param s
     *            student to enroll in the class
     * @throws IllegalArgumentException
     *             if the student is null value, roll throws an exception, and if
     ...
     * 
     * @param s
     *            student to drop from the class
     * @throws IllegalArgumentException
     *             if the student does not exist, or if an index is called not in
     *             list
     */
    public void drop(Student s) {
        ...
        try {
                ...
                if (s.equals(roll.get(i))) {
                    ...
                    if (getNumberOnWaitlist() > 0) {
                        ...
                        if (newStudent.canAdd(course) && canEnroll(newStudent)) {
                            ...
                            newStudent.getSchedule().addCourseToSchedule(course); // add the course to the newly
                                                                                  // enrolled student's schedule
                        } else {
                            throw new IllegalArgumentException(
                                    "First student on waitlist is either already enrolled, they have the course in "
            ...
            for (int i = 0; i < getNumberOnWaitlist(); i++) {
                ...
                if (!s.equals(other)) {
                    waitlist.enqueue(other); // adds dequeued student back to the waitlist if not equal to parameter
                                             // student
                }
            }
            return; // breaks from method
