...
public class StudentDirectoryTest {
	...
	public void testAddStudent() {
		...
		try {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, "hello", "world", MAX_CREDITS);
		} catch (IllegalArgumentException e) {
	...
	public void testGetStudentById() {
	    ...
	    StudentDirectory sd = new StudentDirectory();
        //Test valid Student 1
        sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
        // Test valid Student 2
        sd.addStudent("Zahir", "King", "zking", "orci.Donec@ametmassaQuisque.com", "pw", "pw", 15);
        
        // Very an ID is found
        assertEquals("zking", sd.getStudentById("zking").getId());
        assertEquals("sdent", sd.getStudentById("sdent").getId());
        
        // Verify if an ID is not found
        assertNull(sd.getStudentById("dsgd"));
        
        // Test Invalid Input: null
        try {
            sd.getStudentById(null);
            fail();
        }
        catch (IllegalArgumentException e) {
